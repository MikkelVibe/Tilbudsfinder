...
class CoopTjekPaperParser
{
    ...
    public function parse(string $rawPayload): ParsedPaperInput
    {
        ...
        $paper = new ParsedPaperInput(
            ...
            sourceUrl: $this->optionalString($catalog, 'source_url') ?? 'https://coop.dk/tilbudsavis/',
            ...
            metadata: array_filter([
                ...
                'incito_enriched_offer_count' => Arr::get($catalog, 'incito_enriched_offer_count'),
    ...
    private function decode(string $rawPayload): array
    {
        try {
            ...
            throw new ScraperParseException('COOP Tjek payload is not valid JSON.', previous: $exception);
        }

        if (! is_array($payload)) {
            throw new ScraperParseException('COOP Tjek payload must decode to an object.');
        }
    ...
    private function arrayValue(array $payload, string $key): array
    {
        ...
        if (! is_array($value)) {
            throw new ScraperParseException("COOP Tjek payload is missing {$key}.");
        }
    ...
    private function sourceUnitPrice(array $offer): ?string
    {
        ...
        if ($description === null || preg_match('/(?:Literpris|Kg-pris|Stk\.-pris|Pr\.\s*(?:liter|kg|stk\.))\s*(?:maks\.\s*)?(?<price>\d+(?:[,.]\d+)?)/iu', $description, $matches) !== 1) {
            return null;
        }
    ...
    private function isConditional(array $offer): bool
    {
        ...
        return $description !== null && preg_match('/\b(?:app-pris|medlemspris|medlems-pris|kun med app|kun i app)\b/iu', $description) === 1;
    }
    ...
    private function validateQuality(ParsedPaperInput $paper): void
    {
        if (count($paper->offers) < self::MINIMUM_PARSED_OFFERS) {
            throw new ScraperParseException('COOP Tjek paper must contain at least '.self::MINIMUM_PARSED_OFFERS.' parsed offers.');
        }
        ...
        throw new ScraperParseException('COOP Tjek paper produced zero publishable offers.');
    }
