...
class FoetexPaperParser
{
    ...
    public function parse(string $rawPayload): ParsedPaperInput
    {
        ...
        $paper = new ParsedPaperInput(
            ...
            sourceUrl: 'https://www.foetex.dk/tilbudsavis/',
    ...
    private function decode(string $rawPayload): array
    {
        try {
            ...
            throw new ScraperParseException('føtex payload is not valid JSON.', previous: $exception);
        }

        if (! is_array($payload)) {
            throw new ScraperParseException('føtex payload must decode to an object.');
        }
    ...
    private function arrayValue(array $payload, string $key): array
    {
        ...
        if (! is_array($value)) {
            throw new ScraperParseException("føtex payload is missing {$key}.");
        }
    ...
    private function parseOffers(array $offers): array
    {
        ...
        foreach ($offers as $index => $offer) {
            if (! is_array($offer)) {
                throw new ScraperParseException("føtex offer at index {$index} must be an object.");
            }
    ...
    private function validateQuality(ParsedPaperInput $paper): void
    {
        if (count($paper->offers) < self::MINIMUM_PARSED_OFFERS) {
            throw new ScraperParseException('føtex paper must contain at least '.self::MINIMUM_PARSED_OFFERS.' parsed offers.');
        }
