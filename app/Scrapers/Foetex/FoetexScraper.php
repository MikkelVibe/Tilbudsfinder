...
class FoetexScraper implements GrocerScraper
{
    private const TJEK_API_URL = 'https://squid-api.tjek.com/v2';

    private const TJEK_DEALER_ID = 'bdf5A';
    ...
    public function discoverPapers(?callable $progress = null): array
    {
        ...
        $this->progress($progress, 'Found '.count($catalogs).' active føtex catalogs.');

        $weeklyCatalogs = array_values(array_filter($catalogs, fn (array $catalog): bool => $this->isWeeklyCatalog($catalog)));
        $this->progress($progress, 'Found '.count($weeklyCatalogs).' active føtex Uge catalogs eligible for publishing.');
    ...
    private function fetchActiveCatalogs(): array
    {
        ...
        if (! is_array($catalogs)) {
            throw new ScraperFetchException('føtex catalog response was not an array.');
        }
    ...
    private function fetchCatalogOffers(string $catalogId, ?int $limit): array
    {
        ...
        do {
            ...
            if (! is_array($page)) {
                throw new ScraperFetchException("føtex offers response for catalog {$catalogId} was not an array.");
            }
        ...
        if ($offers === []) {
            throw new ScraperFetchException("føtex catalog {$catalogId} returned no offers.");
        }
    ...
    private function requiredString(array $payload, string $key): string
    {
        ...
        if (! is_string($value) || trim($value) === '') {
            throw new ScraperFetchException("føtex source is missing {$key}.");
        }
    ...
    private function encode(array $payload): string
    {
        try {
            ...
            throw new ScraperFetchException('føtex payload could not be encoded.', previous: $exception);
        }
    }
}
